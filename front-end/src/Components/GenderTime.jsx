...
function GenderTimeQuery() {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:8080/advance/gender-time") // Replace with your actual API endpoint
      .then((response) => {
        ...
        const maleData = months.map((month) =>
          data
            .filter((item) => item.MONTH === month && item.SEX === "M")
            .reduce((acc, curr) => acc + curr.VICTIMCOUNT, 0)
        );

        const femaleData = months.map((month) =>
          data
            .filter((item) => item.MONTH === month && item.SEX === "F")
            .reduce((acc, curr) => acc + curr.VICTIMCOUNT, 0)
        );

        const unknownData = months.map((month) =>
          data
            .filter((item) => item.MONTH === month && item.SEX === "X")
            .reduce((acc, curr) => acc + curr.VICTIMCOUNT, 0)
        );
    ...
    <div>
      {isLoading ? (
        ...
        <>
          ...
          <p>
            This graph shows that on average, throughout the crimes from January
            2020, to between April and May of 2024, male victims have
            experienced more crimes against them. The most crimes that occurred
            against males, was in June 2022, at 8309 crimes. The most crimes
            that occurred against females, was in October 2022, at 7869 crimes.
            One noticeable trend is that during the years 2020 and 2021, the gap
            between the number of crimes against males and females was
            significant, about 1000 crimes separated the two. During 2022, this
            gap narrowed and the two were separated by about 500 crimes at most.
            In 2023, this gap widened again. The crimes where the sex is unknown
            is much less than the crimes where the sex is known and/or reported.
            The difference across almost all the months shown for these crimes
            is about 3000-4000 crimes.
          </p>
        </>
      )}
    </div>
