...
function DataPage() {
  ...
  const center = {
    lat: 34.0522, // Set your desired latitude
    lng: -118.2437, // Set your desired longitude
  };
  ...
  const apiCall = (type) => {
    ...
    axios.post("http://localhost:8080/data", data).then((results) => {
      ...
      for (let index = 0; index < results.data.length; index++) {
        ...
        if (results.data[index][targetColumn] == null)
          results.data[index][targetColumn] = "U";
  ...
  return (
    <div className="flex h-screen">
      <div className="flex flex-col w-64 p-4 text-white bg-gray-800">
        ...
        <button
          onClick={() => apiCall("SEX")}
          className="px-4 py-2 mb-2 bg-gray-700 rounded hover:bg-gray-600"
        >
          Pie Victim Sex
        </button>
        <button
          onClick={() => apiCall("CRIME")}
          className="px-4 py-2 mb-2 bg-gray-700 rounded hover:bg-gray-600"
        >
          Line Crime
        </button>
        <button
          onClick={() => apiCall("RACE")}
          className="px-4 py-2 mb-2 bg-gray-700 rounded hover:bg-gray-600"
        >
          Pie Victim Race
        </button>
        <button
          onClick={() => apiCall("STATUS")}
          className="px-4 py-2 mb-2 bg-gray-700 rounded hover:bg-gray-600"
        >
          Pie Status
        </button>
        ...
        <div>
          ...
          <input
            type="text"
            placeholder="Search..."
            className="w-full p-2 mt-1 text-white bg-gray-700 rounded focus:outline-none"
          />
        </div>
            ...
            {["Type 1", "Type 2", "Type 3"].map((type) => (
              <label key={type} className="inline-flex items-center mt-1">
                <input
                  ...
                  name="type"
                  value={type}
                  className="text-white bg-gray-700 form-radio"
      ...
      <div className="flex-1 p-4 bg-gray-100">
        ...
        <div className="p-4">
          <h2 className="mb-4 text-xl font-bold">Data List</h2>
